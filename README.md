...
## Деплой на сервер (Docker + автообновление)

Для боевого запуска нужен сервер, работающий круглосуточно (это долгоживущий
процесс: Telegram long polling + ежедневный джоб), а не разовый скрипт.
Полная инструкция — в [`docs/DEPLOY.md`](docs/DEPLOY.md): Docker-деплой,
опциональное монтирование Google Диска под фото товара (`rclone mount`) и
systemd-таймер, который сам подтягивает новые коммиты и пересобирает
контейнер.
